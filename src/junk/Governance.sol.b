...
contract Governance is Policy, IGovernance {
    ...
    function endorseProposal(uint256 proposalId_) external {
        ...
        uint256 previousEndorsement = userEndorsementsForProposal[proposalId_][
            msg.sender
        ];
    ...
    function executeProposal() external {
        ...
        if (
            block.timestamp <
            activeProposal.activationTimestamp + EXECUTION_TIMELOCK
        ) {
        ...
        address proposer = getProposalMetadata[activeProposal.proposalId]
            .submitter;
        VOTES.mintTo(
            proposer,
            (VOTES.totalSupply() * GOVERNANCE_BOUNTY) / 10000
        );
